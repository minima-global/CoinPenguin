...
const IntegrityCheck = () => {
  ...
  const handleSubmit = (e) => {
    ...
    (window as any).MDS.cmd(
      `mysql action:integrity host:${formData.host} database:${formData.database} user:${formData.user} password:${formData.password}`,
      (response: any) => {
  ...
  if (!_promptIntegrityCheck) {
    return null;
  }

  return (
    <Dialog dismiss={promptIntegrityCheck}>
          ...
          {step === 0 && (
            <>
              <p className="text-sm mt-4">
                ...
                to check that there are no missing blocks in the MySQL database.
              </p>
              <div className="mt-4">
                <form className="grid gap-1" onSubmit={handleSubmit}>
                  ...
                  <button disabled={loading} className="mt-3 w-full">
                    Submit
                  </button>
                  <button
                    className="mt-2 w-full mx-auto"
                    onClick={promptIntegrityCheck}
                  >
                    Dismiss
                  </button>
                </form>
          ...
          {step === 1 && (
            <>
              <div className="mt-4 bg-white dark:bg-black dark:text-slate-300">
                ...
                <div className="mt-4 bg-black dark:bg-white text-black dark:text-slate-300 flex justify-between items-center px-4 py-4">
